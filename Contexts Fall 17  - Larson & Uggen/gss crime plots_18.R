...
#############################
gss <- read.dta(file="C:/Users/DELL/Documents/UMN/GSS_stata/GSS7218_R1.dta", convert.factors = F,
                missing.type = F)
...
ucr <- read.csv(file="C:/Users/DELL/Documents/UMN/UMN Projects/GSS Crime Trends/UCR crime 1975-2018.csv")
...
bjs <- read.csv(file="C:/Users/DELL/Documents/UMN/UMN Projects/GSS Crime Trends/bjs_incarceration.csv")

ex <- read.csv(file="C:/Users/DELL/Documents/UMN/UMN Projects/GSS Crime Trends/execution_database_19.csv", stringsAsFactors = F)
...
ggplot(fear.gen)+  
  geom_line(aes(x=year, y=perc, group=sex), size=.5)+
  ...
  ggtitle("GSS Crime Fears by Gender, 1975-2018", subtitle = "Afraid to walk at night in neighborhood?")+
  labs(colour="UCR", fill="GSS: Fear")+
  scale_x_continuous(limits=c(1975,2018), breaks=seq(1975,2018,5))+
  scale_y_continuous(sec.axis=sec_axis(~.*60))+
  xlab("")+ylab("")+
  theme_hc()+
  theme(legend.position = "none")+
  annotate("text", label="UCR Arrest Rate per 100,000", x=2005, y=85, size=3)+
  annotate("text", label="Male", x=1978, y=30, size=3)+
  annotate("text", label="Female", x=1978, y=72, size=3)
...
ggplot(courts)+  
  geom_line(aes(x=year, y=perc), size=.5)+
  geom_line(aes(x=year, y=rate/10, col="imprisonment \n per 100,000"), size=.5)+
  geom_ribbon(aes(x=year, ymin=ci.l, ymax=ci.h), alpha=.5)+
  ggtitle("GSS Courts, 1975-2018", subtitle="Are Courts too Harsh?")+
  scale_x_continuous(limits=c(1975,2018), breaks=seq(1975,2018,5))+
  scale_y_continuous(limits=c(0,80),sec.axis=sec_axis(~.*10))+
  xlab("")+ylab("")+
  theme_hc()+
  theme(legend.position = "none")+
  annotate("text", label="BJS Incarceration per 100,000", x=1984, y=51, size=3)+
  annotate("text", label="GSS Percent", x=1983, y=8, size=3)
...
pol <- pol %>% filter(race!="Other")

ggplot(pol)+  
  geom_line(aes(x=year, y=perc, group=race), size=.5)+
  geom_ribbon(aes(x=year, ymin=ci.l, ymax=ci.h, fill=race), alpha=.5)+
  ggtitle("GSS Police Force by Race, 1975-2018", subtitle="Approve of policeman striking male citizen?")+
  labs(fill="GSS: Force")+
  scale_x_continuous(limits=c(1975,2018), breaks=seq(1975,2018,5))+
  ...
  theme_hc()+
  theme(legend.position = "none")+
  annotate("text", label="White", x=1980, y=90, size=3)+
  annotate("text", label="Black", x=1977, y=35, size=3)
  #annotate("text", label="Other", x=1979, y=70, size=3)
...
ggplot(cap.pol)+  
  geom_line(aes(x=year, y=perc, group=party), size=.5)+
  geom_line(aes(x=year, y=exec, col="executions"), size=.5)+
  scale_color_manual(values=c("black"))+
  geom_ribbon(aes(x=year, ymin=ci.l, ymax=ci.h, fill=party), alpha=.5)+
  ggtitle("GSS Capital Punishment by Party ID, 1975-2018", subtitle = "Do you favor or oppose the death penalty for persons convicted of murder?")+
  ...
  theme_hc()+
  theme(legend.position = "none")+
  annotate("text", label="DPIC Executions", x=1998, y=30, size=3)+
  annotate("text", label="Democrat", x=2016, y=42, size=3)+
  annotate("text", label="Independent, other", x=2014, y=73, size=3)+
  annotate("text", label="Republican", x=2013, y=93, size=3)
